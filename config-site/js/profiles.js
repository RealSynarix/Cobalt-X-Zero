...
function readStore(){try{const raw=localStorage.getItem(KEY);if(!raw)return {};return JSON.parse(raw);}catch{return {};}}
...
if(name.trim().length>32)throw new Error("Name too long max 32");
...
export function getActiveName(){return localStorage.getItem(ACTIVE_KEY)||"";}
...
listDiv.innerHTML='<div class="muted">No profiles yet</div>';
return;
}
profiles.forEach(p=>{
...
row.innerHTML=`<div class="row" style="justify-content:space-between"><span>${p.name}</span><span class="muted">${new Date(p.data.savedAt||Date.now()).toLocaleDateString()}</span></div>`;
