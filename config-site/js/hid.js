// js/hid.js - WebHID handling for Cobalt-X Zero
// Packet: 64B [cmd, fieldId, len_lo, len_hi, payload...]
// Report ID 2 preferred. Fallback order per spec.
...
export async function sendPacketAndReadResponse(packet) {
  ...
  let sent = false;
  const reportData = toSend;
  // try sendReport(2), then sendReport(0), then sendFeatureReport(2)
  try {
    await device.sendReport(REPORT_ID, reportData);
    sent = true;
  } catch {}
  if (!sent) {
    try { await device.sendReport(0, reportData); sent = true; } catch {}
  }
  if (!sent) {
    try { await device.sendFeatureReport(REPORT_ID, reportData); sent = true; } catch {}
  }
  if (!sent) throw new Error("Failed to send report");
  ...
  try {
    ...
    respRaw = new Uint8Array(fr.buffer || fr);
  } catch {
    // fallback try report 0
    try {
      const fr2 = await device.receiveFeatureReport(0);
      respRaw = new Uint8Array(fr2.buffer || fr2);
    } catch {}
  }
  if (!respRaw) throw new Error("No response");
  return parseResponse(respRaw);
}

export async function readField(fieldId) {
  const pkt = makePacket(CMD_READ, fieldId, new Uint8Array(0));
  const res = await sendPacketAndReadResponse(pkt);
  if (res.fieldId !== fieldId) {
    // allow but warn
  }
  return res;
}
