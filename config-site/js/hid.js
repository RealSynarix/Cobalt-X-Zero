...
const CMD_READ=0x01;const CMD_WRITE=0x02;const CMD_GET_ALL=0x03;const CMD_RESET=0x04;const CMD_PING=0x05;
...
device=d;connected=true;return true;
}catch(e){throw e;}
}
...
const cmd=data[off];const fid=data[off+1];const len=data[off+2]|(data[off+3]<<8);const status=data[off+4];
const payload=data.slice(off+5,off+5+len);
return {cmd,fieldId:fid,len,status,payload,raw:data};
}
...
let resp=null;
try{const fr=await device.receiveFeatureReport(REPORT_ID);resp=new Uint8Array(fr.buffer||fr);}catch{
try{const fr2=await device.receiveFeatureReport(0);resp=new Uint8Array(fr2.buffer||fr2);}catch{}
}
if(!resp)throw new Error("no response");
return parseResponse(resp);
}
export async function readField(fid){const pkt=makePacket(CMD_READ,fid,new Uint8Array(0));return await sendPacketAndReadResponse(pkt);}
export async function writeField(fid,payload){const p=payload instanceof Uint8Array?payload:new Uint8Array(payload);const pkt=makePacket(CMD_WRITE,fid,p);return await sendPacketAndReadResponse(pkt);}
export async function ping(){const pkt=makePacket(CMD_PING,0,new Uint8Array([0xAA,0x55]));return await sendPacketAndReadResponse(pkt);}
...
export function colorToBytes(hex){const h=String(hex).replace('#','').trim();if(!/^[0-9a-fA-F]{6}$/.test(h))throw new Error("invalid color");const r=parseInt(h.slice(0,2),16),g=parseInt(h.slice(2,4),16),b=parseInt(h.slice(4,6),16);return new Uint8Array([r,g,b]);}
