...
static inline void debounce_step(uint8_t idx,uint8_t pin){
  if(locks[idx]!=0){ locks[idx]--; return; }
...
void pipeline_init(void){
  ...
  TIM2->CR1=0u; TIM2->SMCR=3u; TIM2->CCMR1=0xF1F1u; TIM2->ARR=0xFFFFFFFFu; TIM2->CNT=0u; TIM2->CR1=TIM_CR1_CEN;
  last_enc_count=TIM2->CNT;
  macro_init();
}

void pipeline_tick(void){
  debounce_step(0,3); debounce_step(1,4); debounce_step(2,5); debounce_step(3,6);
  ...
  if(frame_tick>=TICKS_PER_FRAME){
    ...
    int32_t steps=0;
    if(wheel_accum!=0){
      steps=wheel_accum/ENCODER_DIVIDER;
